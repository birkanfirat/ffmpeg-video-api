...
async function elevenTtsToFile(text, outPath) {
  ...
  while (true) {
    ...
    // Free plan vs için 429 retry/backoff
...
/** =========================
 *  JOB STORE (in-memory)
 *  Not: tek replica varsayımı; restart olursa job kaybolur.
 *  ========================= */
const jobs = new Map(); // jobId -> {status, dir, output, error, createdAt}

/** plan -> iş listesi üretir (download + tts sırayla) */
function buildJobsFromPlan(plan) {
  ...
  if (plan.useBismillahClip && plan.bismillahAudioUrl) {
    addDL("AR_BISM", plan.bismillahAudioUrl);
  }
  ...
  try {
    ...
    for (const j of work) {
      ...
      if (j.kind === "download") {
        ...
        // boş text gelirse 1 sn sessizlik üretmek yerine atlamıyoruz:
        // ElevenLabs boş text'i bazen reddedebilir, o yüzden guard:
        const t = String(j.text || "").trim();
        const safeText = t.length ? t : " ";
        await elevenTtsToFile(safeText, out);
 ...
 *  multipart: image(binary) + plan(text JSON string)
 *  returns: { jobId }
 *  ========================= */
app.post(
  "/render10min/start",
...
app.get("/render10min/result/:id", (req, res) => {
  ...
  if (st.status === "error") {
    return res.status(500).json({ error: st.error || "job failed" });
  }
...
/** Optional: eski endpoint'e gelenleri yönlendir (yanlış çağrıları yakala) */
app.post("/render10min", (req, res) => {
  return res
    .status(410)
    .json({ error: "Use POST /render10min/start then poll status/result endpoints." });
});
